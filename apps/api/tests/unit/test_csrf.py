...
class TestCSRFToken:
    ...
    def test_validate_wrong_csrf_token(self, fake_redis):
        ...
        assert validate_csrf_token(fake_redis, sid="test-sid-4", token="wrong-token") is False  # noqa: S106

    def test_validate_missing_csrf_token(self, fake_redis):
        from src.middleware.csrf import validate_csrf_token
        assert validate_csrf_token(fake_redis, sid="no-session", token="any") is False  # noqa: S106
